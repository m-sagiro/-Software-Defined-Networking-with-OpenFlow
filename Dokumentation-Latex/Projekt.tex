...
\begin{wrapfigure}{r}{0.4\textwidth}
	...
	\label{layer1}
\end{wrapfigure}
In dieser Projektarbeit werden Netzwerk\hyp{}Funktionen einer Software-Defined Networking Umgebung geprüft und anhand vorgegebener Aufgaben realisiert. Implementiert werden diese Aufgaben mithilfe von Mininet, ein Netzwerkemulator, der ein Netzwerk aus virtuellen Hosts, Switches und Controllern erstellt, und das Controller Framework Floodlight, ein Open Source, Java Openflow Controller.

Durch SDN wird die Software von der Hardware entkoppelt. SDN verlagert die Steuerungsebene, die festlegt, wohin der Datenverkehr gesendet werden soll, in die Softwareebene und lässt die Datenebene, die den Datenverkehr tatsächlich weiterleitet, in der Hardwareebene. Dies ermöglicht es Administratoren, die Software-defined Networking verwenden, das gesamte Netzwerk über einen einzigen Controller zu steuern, anstatt jedes einzelne Gerät zu kontrollieren. Typischerweise besteht eine SDN-Architektur aus drei Teilen:
Die Anwendungsschicht, Kontrollschicht und Infrastrukturschicht. In der Anwendungsschicht werden die Netzwerkanforderungen und das gewünschte Netzwerkverhalten definiert. In der Kontrollschicht wird anhand der Informationen aus der Anwendungsschicht mit dem Controller entschieden, wie ein Datenpaket weitergeleitet werden soll. In der Infrastruktursicht wird anhand der Informationen aus der Kontrollschicht entschieden beziehungsweise des SDN Controllers, wohin die Datenpakete weitergeleitet werden sollen.
Anschließend bewegen Netzwerkgeräte die Daten durch das Netzwerk. Switches prüfen die Integrität der Datenpakete und leiten diese weiter (siehe Abbildung \ref{layer1}).

SDN ist ein Netzwerkansatz, bei dem softwarebasierte Steuerungen verwendet werden, um mit vorhandenen Hardwaregeräten, das heißt mit Routern und Switches, zu kommunizieren und den Datenverkehr in einem Netzwerk zu steuern. Dieses Modell unterscheidet sich von traditionellen Netzwerken, die Hardwaregeräte zur Steuerung des Netzwerkverkehrs verwenden. Durch diesen Unterschied ist SDN viel flexibler als andere hardwarebasierte Netzwerke und ermöglicht Administratoren, das Netzwerk zu steuern, Konfigurationseinstellungen zu ändern, Datenflüsse bereitzustellen und die Netzwerkkapazitäten zu erhöhen. Dies kann alles über eine zentrale Benutzeroberfläche umgesetzt werden, ohne dass weitere Hardware benötigt wird. \par
SDN ermöglicht eine andere Art der Steuerung eines Netzwerkes im Gegensatz zu klassischen Netzwerken, wie beispielsweise virtuelle Netzwerke: die Weiterleitung von Datenpaketen über einen zentralen Knotenpunkt. Somit stellt SDN einen wesentlichen Fortschritt gegenüber klassischen Netzwerken dar. Anstatt mehrere herstellerspezifische Hardwaregeräte manuell zu programmieren, können Entwickler den Datenverkehr über ein Netzwerk durch einen vorprogrammierten Controller steuern. Netzwerkadministratoren können auch bei der Anzahl von Netzwerkgeräten flexibler sein, da durch ein einziges Protokoll eine beliebige Anzahl von Hardwaregeräten über einen zentralen Controller kommunizieren können. Mithilfe von SDN können Administratoren auch Netzwerkdienste konfigurieren und Datenflüsse anpassen, demnach können Netzwerkadministratoren den Datenfluss durch das Netzwerk optimieren und Anwendungen, die eine höhere Verfügbarkeit erfordern, priorisieren. Dies und weitere Vorteile von Software-Defined Networking werden in dieser Projektarbeit veranschaulicht.
\section*{Einleitung von Mücahit}\label{einl-müco}
\begin{wrapfigure}{r}{0.4\textwidth}
	\vspace{-\baselineskip}
	\centering
	\includegraphics[width=0.45\textwidth]{Bilder/layer2}
	\caption{SDN Architektur}
	\label{layer2}
\end{wrapfigure}


Software-defined Networking beschreibt einen agilen Ansatz zur Verwaltung und Administrierung von Computernetzwerken aus einer zentralen Stelle. Dabei muss nur an der zentralen Stelle eine Änderung vorgenommen werden, um Änderungen an mehrere Netzwerkkomponenten beziehungsweise -bereichen vorzunehmen. Ein sogenannter Controller stellt eine zentrale Stelle dar und steuert den ihm zugehörigen Netzwerkabschnitt. Switches und Routern leiten Pakete nach Regeln beziehungsweise Tabellen weiter, wo üblicherweise für eine Änderung der Regeln jedes Gerät einzeln angesprochen werden muss. Durch die Software im Controller werden Änderungen angenommen und auf die Netzwerkkomponenten übertragen. Diese Vorgehensweise durch Software-defined Networking erlaubt eine dynamische Anpassung des Computernetzwerkes \cite[1-2]{sdn1}. Durch das Hinzufügen eines Controllers wird die sogenannte Control Plane vom Switch und Router entbunden. Dadurch verbleiben der Switch und der Router in der Data Plane, wo sie weiterhin für die Paketweiterleitung zuständig bleiben. Der Controller hingegen ist nun für die Entscheidungen verantwortlich, wie mit einem Paket verfahren werden soll. Aus der Entkopplung der Control Plane von der Data Plane ergibt sich im Ganzen die sogenannte Application Layer, die Data Plane und dazwischen die Control Plane, was nun Änderungen leichter zulässt (siehe Abbildung \ref{layer2}). Programme können über die application programming interface (API) mit dem Controller kommunizieren und ihm Anweisungen über Pakete oder Netzwerkbereiche geben. Der Controller wiederum wendet die getroffenen Entscheidungen an den zugehörigen Netzwerkkomponenten an \cite[2]{sdn2}. Des Weiteren kann über die API verschiedene Netzwerkleistungen implementiert werden. Darunter zählen neben der Kontrolle des Paketflusses beispielsweise Implementierungen bezüglich der Sicherheit, der Quality of Service (QoS), der Zugriffskontrolle und weitere Leistungen, die für ein Netzwerk wichtig sind \cite[8-9]{sdn2}. Bei klassischen Switches werden anhand eingehender Pakete MAC-Adressen mit dem dazugehörigen Ausgangsport in einer Tabelle hinterlegt. Fehlt diese Information, wird ein Paket auf allen verfügbaren Ports ausgegeben. Hierbei würde ein Switch keine Auslastungen außerhalb seines Bereiches kennen. Aus diesem Grund würde der Switch das Paket auf die für das Ziel vorgesehene Strecke einordnen, die er vorher per Address Resolution Protocol Anfrage (ARP) in seinem Cache abgelegt hat. Der Controller jedoch hat einen Überblick auf das ganze Netzwerk, die von ihm verwaltet wird, weshalb er eine performantere Entscheidung treffen kann, wohin das Paket am besten weitergeleitet werden sollte, um beispielsweise Überlastungen zu vermeiden \cite[1-2]{sdn1}. Über ein im Vergleich zu Hypertext Transfer Protocol (HTTP) junges Protokoll namens OpenFlow (OF) kommuniziert der Controller mit bestimmten Netzwerkkomponenten wie beispielsweise ein programmierbarer Switch, um diese zu steuern. Es gibt zwei Arten von programmierbaren Switches: OpenFlow-only und OpenFlow-enabled Switches. OpenFlow-enabled Switches sind klassische Switches, die zusätzlich OpenFlow fähig sind. OpenFlow-only Switches haben keine zusätzlichen Module eingebaut, auf die Sie zurückgreifen können und müssen deswegen an einen Controller gebunden sein, der dann beispielsweise entscheidet, wohin eingehenden Pakete weiterleitet werden sollen. Dafür sendet der Switch ein Paket oder ein Teil eines Paketes an den Controller und wartet auf einen Befehl, wie er sich zu verhalten hat. Falls der Switch vorher einen Eintrag über das Paket besitzt, greift er auf diese zurück, und wenn überhaupt kein Eintrag vorhanden ist, wird das Paket nach den Einstellungen der Switch entweder verworfen, an allen Ports ausgegeben oder an den Controller gesendet. Die Fähigkeit, bestimmte Befehle auf Pakete anzuwenden, die auf einer Übereinstimmung basieren, zeichnet Software-defined Networking aus \cite[9-10]{sdn2}. Der Controller stellt einen sogenannten Single Point of Failure dar. Das bedeutet, wenn der Controller ausfällt, fallen alle vom Controller abhängigen Netzwerkkomponenten ebenfalls aus. So würde beispielsweise bei einer Denial-of-Service-Attacke durch das Überfluten des Controllers ein Zustand erfolgen, bei dem der Controller an Bandbreite, Speicher und Rechenleistung verlieren und im schlimmsten Fall auch lahmgelegt werden kann. Das ist eines der schlimmsten Szenarien und könnte durch das Hinzufügen eines zweiten Controllers kompensiert werden \cite[3]{sdn3}.
...
\subsection{Vorüberlegung}
{\tiny Verfasst von: Mücahit\par}
Die NAT-Firewall Funktion muss am Router einer Lokation implementiert werden, da alle Hosts über ihn Anfragen ins Internet verschicken werden. Dazu sollte jede Anfrage ins World Wide Web mit der öffentlichen IPv4-Adresse des Routers durchgeführt werden. Zusätzlich sollte der Router beziehungsweise die Hosts den Domain Name System (DNS) Server konfiguriert bekommen, damit die Hosts nicht nur über die IPv4-Adressen aufs Internet zugreifen.
