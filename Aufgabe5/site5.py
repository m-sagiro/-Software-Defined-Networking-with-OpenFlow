...
# Hierzu wird IPv4 Forwarding aktiviert
# Beim terminieren wird Ipv4 Forwarding deaktiviert
# und die Ubuntu-Netzwerkschnittstelle zurückgewiesen (WICHTIG!)
...
#    Intf('enp0s10', node=s2)
    ...
    # Internet connection für r1-r4 sorgen!
    r1 = net.getNodeByName('r1')
    Intf('enp0s3', node=r1)
    info(net['r1'].cmd("dhclient enp0s3"))
    info(net['r1'].cmd("sudo iptables -t nat -A POSTROUTING -o enp0s3 -j MASQUERADE"))

#    r2 = net.getNodeByName('r2')
#    Intf('enp0s8', node=r2)
#    info(net['r2'].cmd("dhclient enp0s8"))
#    info(net['r2'].cmd("sudo iptables -t nat -A POSTROUTING -o enp0s8 -j MASQUERADE"))

#    r3 = net.getNodeByName('r3')
#    Intf('enp0s9', node=r3)
#    info(net['r3'].cmd("dhclient enp0s9"))
#    info(net['r3'].cmd("sudo iptables -t nat -A POSTROUTING -o enp0s9 -j MASQUERADE"))

#    r4 = net.getNodeByName('r4')
#    Intf('enp0s10', node=r4)
#    info(net['r4'].cmd("dhclient enp0s10"))
#    info(net['r4'].cmd("sudo iptables -t nat -A POSTROUTING -o enp0s10 -j MASQUERADE"))
    ...
    info(net['s1'].cmd("ovs-vsctl set port s1-eth1 qos=@newqos -- --id=@newqos create qos type=linux-htb other-config:max-rate=20000000 queues:0=@newqueue0 queues:1=@newqueue1 -- --id=@newqueue0 create queue other-config:max-rate=20000000 -- --id=@newqueue1 create queue other-config:min-rate=6000000 other-config:max-rate=6000000"))
    ...
    # Give INterfaces back when we stop -> Stop means going out of CLI
    info(net['r1'].cmd("ip link set enp0s3 netns 1"))
#    info(net['r2'].cmd("ip link set enp0s8 netns 1"))
#    info(net['r3'].cmd("ip link set enp0s9 netns 1"))
#    info(net['r4'].cmd("ip link set enp0s10 netns 1"))

#    info(net['s1'].cmd("ip link set enp0s9 netns 1"))
#    info(net['s2'].cmd("ip link set enp0s10 netns 1"))
